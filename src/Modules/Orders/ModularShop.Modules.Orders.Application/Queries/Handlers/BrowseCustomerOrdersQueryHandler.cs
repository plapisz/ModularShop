...
{
    public async Task<IReadOnlyCollection<OrderDto>> HandleAsync(BrowseCustomerOrdersQuery query)
        => await orderReadRepository.BrowseByCustomerIdAsync(query.CustomerId);
}